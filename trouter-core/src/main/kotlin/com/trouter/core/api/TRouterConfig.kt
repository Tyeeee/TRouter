 ...
 *
 * 拦截器版本 新增：
 * - [interceptors]：拦截器列表（顺序即执行顺序，见 RouteInterceptor）；默认空列表 = 行为与 最早的基础版本 一致。
